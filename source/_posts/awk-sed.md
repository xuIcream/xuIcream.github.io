title: awk sed
date: 2016-01-10 12:17:25
tags: unix
---

86451|{"language":[{"name":"java","version_name":7}, {"name":"c","version_name":8}, {"name":"python","version_name":2}],"level":42,"time":1452008786167,"board":"aaaa"}
86451|{"language":[{"name":"java","version_name":7}, {"name":"c","version_name":8}, {"name":"python","version_name":2}],"level":42,"time":1452008786167,"board":"bbbb"}

for board in "aaaa" "bbbb"
do
    echo $board
    cat $1 | sed -n "/$board/p" | awk -F '\\|{' '{print "{\"number\":"$1 "," $2}' | jq '{number, time, board, language : .language[]}' | jq '[.number, .time, .board, .language.name, .language.version_name]' -c > ${1}_$board
done

sed [原文地址](http://coolshell.cn/articles/9104.html)
sed -n "/s/p" 　输入匹配行 
在sed中引入shell变量 sed "s/$a/$b/" filename

awk [原文地址](http://coolshell.cn/articles/9070.html)
